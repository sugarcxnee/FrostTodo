...
简约、冷色调的原生 macOS 待办与计时应用。支持任务管理、任务计时、倒计时（类番茄钟）、时间段记录、完整历史追溯，并与系统日历（Apple 日历 / iCloud 日历）联动：计时开始、暂停、结束、任务完成时自动创建或更新日历事件。数据全部保存在本机，不上传服务器。
...
- 任务管理：收件箱、今天、计划、已完成、标签、项目视图；快速添加、编辑、删除、完成与取消完成；手动拖拽排序；搜索与四种排序（手动、创建时间、截止日期、优先级）；任务详情修改自动保存（0.4 秒防抖，连续输入合并为一条编辑历史，无实际变更不写历史）。
...
- 设置：写入日历开关、默认日历选择、开始即建事件与完成事件开关、默认预计时长、倒计时默认专注/休息时长与轮数、外观（跟随系统、浅色、深色）、历史保留策略、清除已完成任务、清空历史、导出 JSON。
...
│   ├── Protocols/                 ClockProviding、HistoryRecording、CalendarProviding、NotificationCentering、TimerObserving、CountdownCoordinating
│   ├── Services/                  Persistence、Task、Timer、Countdown、History、CalendarSync、EventKit、Mock、Notification、Settings、ShortcutRouter、Export
│   ├── ViewModels/                App、TaskList、History、Settings、Timer、Countdown
│   └── Views/                     Theme、MainView 三栏、Sidebar（计时面板）、TaskList、TaskDetail、Timer、History、Settings、MenuBar
└── Tests/FrostTodoTests/          按阶段组织的 23 个测试套件，共 153 例
```
...
```
Test run with 153 tests in 23 suites passed after 0.527 seconds.
```
...
| 3 历史记录 | Phase3HistoryServiceTests / Phase3BusinessIntegrationTests / Phase3HistoryExportTests / AutosaveContractTests | 32 |
...
| 7 倒计时 | Phase7CountdownTests（引擎、ViewModel、任务级时长、轮数、生命周期联动五个套件） | 36 |
...
2. 计时恢复基于“快照 + 数据推导”：TimerSnapshot 记录 activeTaskID、phase 与本轮开始时间，累计时长总是从该时间之后已结束的 Session 求和，崩溃产生的脏快照（Session 已结束但快照仍为 running）在恢复时安全降级为暂停；倒计时同理，剩余时间由 targetEndAt 推导。
3. 事务语义：业务操作与历史写入在同一逻辑单元内一次保存，任一失败即回滚上下文并恢复模型内存状态（ModelContext.rollback 不还原模型内存属性，需手动捕获恢复），保证“操作成功必有历史、失败无残留”。日历同步属于外部副作用，失败不回滚本地操作，改记 calendar.syncFailed 历史。
4. 倒计时建立在正计时之上（专注时间即 Session）：阶段转换复用暂停/继续机制，因此日历与历史记录天然一致；正计时以任何方式结束时经 CountdownCoordinating 通知倒计时同步结束，防止悬空计时。
5. 智能视图定义：收件箱为无开始且无截止日期的未完成任务；今天为已开始或今日到期的未完成任务；计划为开始日期在未来的未完成任务。
...
- 阶段转换在每秒 tick 时发生：倒计时到点后的阶段切换最多延迟一秒。
- 数据导出 JSON 暂未提供导入功能；一致性校验通过解码往返完成。
- 通过 `swift run` 运行时无沙盒与权限描述，日历联动不可用，请使用 Xcode 工程运行。
- 界面文案为简体中文，未做国际化。
- MenuBarExtra 与计时面板的计时文字每秒刷新一次，长时间运行可能有小幅 CPU 占用。
